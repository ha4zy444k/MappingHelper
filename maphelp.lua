...
local function checkForUpdatesAsync()
    ...
    lua_thread.create(function()
        ...
        if not downloadOk then
            logDebug("Update", "warn", "РќРµ СѓРґР°Р»РѕСЃСЊ СЃРєР°С‡Р°С‚СЊ version.json СЃ GitHub")
        ...
        if not f then
            logDebug("Update", "error", "РќРµ СѓРґР°Р»РѕСЃСЊ РѕС‚РєСЂС‹С‚СЊ РІСЂРµРјРµРЅРЅС‹Р№ С„Р°Р№Р» version.json")
            return
        end
        ...
        if not ok or type(data) ~= "table" then
            logDebug("Update", "error", "РћС€РёР±РєР° РїР°СЂСЃРёРЅРіР° version.json: " .. tostring(content):sub(1, 100))
            return
        end
        ...
        if not remoteVersion then
            logDebug("Update", "warn", "version.json РЅРµ СЃРѕРґРµСЂР¶РёС‚ РїРѕР»Рµ 'version'")
            return
        end
        ...
        if isVersionNewer(remoteVersion, scriptVersion) then
            ...
            logDebug("Update", "success", "Р”РѕСЃС‚СѓРїРЅР° РЅРѕРІР°СЏ РІРµСЂСЃРёСЏ: " .. remoteVersion)
        else
            logDebug("Update", "info", "Р’РµСЂСЃРёСЏ Р°РєС‚СѓР°Р»СЊРЅР°: " .. scriptVersion)
        end
    end)
end
...
local function performScriptUpdate()
    ...
    downloadUrlToFile(updateState.downloadUrl, tempPath, function(id, status, p1, p2)
        if status == dlstatus.STATUS_DOWNLOADINGDATA and p2 and p2 > 0 then
            updateState.downloadStatus = u8("Р—Р°РіСЂСѓР·РєР°: ") .. math.floor((p1 / p2) * 100) .. "%"
